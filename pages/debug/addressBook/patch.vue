...
import type { PatchRecipientPayload } from '~/lib/beta/addressBookApi'
...
const makeApiCall = async () => {
  loading.value = true
  const payloadData: PatchRecipientPayload = {
    metadata: {
  ...
  try {
    await $addressBookApiBeta.patchRecipient(formData.recipientId, payloadData)
  } catch (err) {
    error.value = err
